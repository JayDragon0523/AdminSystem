...
public interface ScheduleDAO extends JpaRepository<Schedule,BigInteger> {
    ...
    //预约会议后修改会议室时间片状态
    @Modifying
    @Query("update Schedule set state='已预约' where placeId =?1 and schedule between ?2 and ?3 and companyId=?4 and dayOfWeek=?5")
    void updateStateForAppointment(BigInteger placeId,String start,String end,BigInteger companyId,String dayOfWeek);

    //取消预约会议后修改会议室时间片状态
    @Modifying
    @Query("update Schedule set state='空闲' where placeId =?1 and schedule between ?2 and ?3 and dayOfWeek=?4")
    void updateStateForCancelAppointment(BigInteger placeId,String start,String end,String dayOfWeek);
